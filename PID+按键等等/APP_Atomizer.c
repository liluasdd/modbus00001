...
u16	R_Res_average_QQ=d_Res_max; 
...
//uc8	R_read_haveL[7]={5,10,20,30,40,50,75};
uc8	R_read_haveL[7]={10,20,30,40,50,60,85};
u8	R_haveL_I=0;
...
u16 Read_L_1C(void)
{
	...
	for(i=0;i<=d_read_haveLMAX;i++)
	{
		...
		{
			B=0;
			if(Read_outI_Data_1C()>300)
			{
				Out_V=Read_outV_Data1();
				if(Out_V<120||Out_V>190)
				{
					break;
				}
			}
...
void	Enable_VoutADC_Use(void)
{
	B_VoutADC_Use=1;
	/*配置成ADC口*/
    gpio_mode_set(SEN_VOUT_GPIO_Port, SEN_VOUT_Pin, GPIO_MODE_ANALOG);	
}
...
void	Disable_VoutADC_Use(void)
{
	...
	B_VoutADC_Use=0;
	/*配置成普通IO口*/
	 gpio_mode_set(SEN_VOUT_GPIO_Port, SEN_VOUT_Pin, GPIO_MODE_OUT_PP(GPIO_SPEED_HIGH));  
      SEN_VOUT_GPIO_Port->DO |= SEN_VOUT_Pin;	
}
			...
			if(B_VoutADC_Use)
			{
				...
				else
				{
					...
					if(LR_i>=12)
					{
							...
							if(R_Res_average<(R_Res_average_QQ*102/100)&&R_Res_average>(R_Res_average_QQ*98/100))
							     R_Res_average=R_Res_average_QQ;
...
Load_Data	*Atomizer_Pointer[]={&Load_Ss316,&Load_Ni200,&Load_Ti,};
