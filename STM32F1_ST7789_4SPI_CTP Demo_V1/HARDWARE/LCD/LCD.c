...
void LCD_Display_Dir(u8 dir)
{
	if (dir == 0 || dir == 1) // 竖屏
	{
		...
		{
			LCD_WR_REG(0x36);
			LCD_WR_DATA((0 << 3) | (0 << 7) | (0 << 6) | (0 << 5)); // 0-0度旋转
		}
		else // 1-180度旋转
		{
			LCD_WR_REG(0x36);
			LCD_WR_DATA((0 << 3) | (1 << 7) | (1 << 6) | (0 << 5)); // 1-180度旋转
		}
	}
	else if (dir == 2 || dir == 3)
	{
...
void LCD_Init(void)
{
	...
	//==== GPIOA配置 PA4-PA7 ====
	GPIOA->CRL &= 0X0000FFFF; // 清除PA4-PA7
	// GPIOA->CRL |= 0X33430000;					  // PA4=DC, PA5=SCK推挽输出; PA6=SDO浮空输入; PA7=SDI推挽输出
	// GPIOA->CRL |= 0X33440000;					  // PA4=DC, PA5=SCK推挽输出; PA6=SDO浮空输入; PA7=SDI推挽输出
	// GPIOA->CRL |= 0X33340000;					  // PA4=3, PA5=3, PA6=4, PA7=3
	GPIOA->CRL |= 0X33330000;
	GPIOA->ODR |= (1 << 4) | (1 << 5) | (1 << 7); // PA4,5,7上拉

	//==== GPIOB配置 PB0,PB1 ====
	GPIOB->CRL &= 0XFFFFFF00;		   // 清除PB0,PB1
	GPIOB->CRL |= 0X00000033;		   // PB0=LED, PB1=CS 推挽输出50MHz
	GPIOB->ODR |= (1 << 0) | (1 << 1); // PB0,PB1上拉

	//==== GPIOB配置 PB10-PB14 ====电容触摸屏
	GPIOB->CRH &= 0X000FFFFF; // 清除PB15-PB10
	// GPIOB->CRH |= 0X333BB000;									 // PB10=CLK, PB11=MOSI, PB12=CS, PB13=RST推挽输出; PB14=INT浮空输入
	// GPIOB->CRH |= 0X33334000;									 // PB14=INT浮空输入
	GPIOB->CRH |= 0X33330000;									 // PB10, PB11, PB12, PB13 推挽输出50MHz
	GPIOB->CRH |= 0X00040000;									 // PB14 浮空输入
	GPIOB->ODR |= (1 << 10) | (1 << 11) | (1 << 12) | (1 << 13); // PB10-13上拉
	...
	LCD_WR_REG(0x36);  //
	LCD_WR_DATA(0x00); // 0度旋转
	// LCD_WR_DATA((0 << 3) | (1 << 7) | (1 << 6) | (0 << 5));// 180度旋转
	LCD_WR_REG(0x3a);
	LCD_WR_DATA(0x05);
