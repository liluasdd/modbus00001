...
u8 FT6336_Scan(u8 mode)
{
	...
	if ((t % 10) == 0 || t < 10) // 空闲时,每进入10次CTP_Scan函数才检测1次,从而节省CPU使用率
	{
		...
		if ((mode & 0XF) && ((mode & 0XF) < 6))
		{
			...
			tp_dev.sta = (~temp) | TP_PRES_DOWN | TP_CATH_PRES; // 更新触摸屏状态
			for (i = 0; i < 5; i++)
			{
				if (tp_dev.sta & (1 << i)) // 触摸有效?
				{
					FT6336_RD_Reg(FT5206_TPX_TBL[i], buf, 4); // 读取XY坐标值
					if (tp_dev.touchtype & 0X01)			  // 0度竖屏
					{
						tp_dev.y[i] = ((u16)(buf[0] & 0X0F) << 8) + buf[1]; // y
						tp_dev.x[i] = ((u16)(buf[2] & 0X0F) << 8) + buf[3]; // x
					}
					else // 90度横屏
					{
					...
					// if (condition) // 180度竖
					// {
					// 	tp_dev.y[i] = 240 - ((u16)(buf[0] & 0X0F) << 8) + buf[1]; // y
					// 	tp_dev.x[i] = 320 - ((u16)(buf[2] & 0X0F) << 8) + buf[3]; // x
					// }
					// else // 270度横屏
					// {
					// 	tp_dev.x[i] = 320 - (((u16)(buf[0] & 0X0F) << 8) + buf[1]); // x
					// 	tp_dev.y[i] = ((u16)(buf[2] & 0X0F) << 8) + buf[3];			// y
					// }
