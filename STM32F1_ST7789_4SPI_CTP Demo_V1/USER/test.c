...
int main(void)
{
	...
	// // 测试1：什么都不初始化，直接设置PB14
	// RCC->APB2ENR |= 1 << 3; // 使能GPIOB时钟，添加了寄存器详细说明（APB2总线外设时钟使能寄存器，位3控制GPIOB）
	// GPIOB->CRL &= 0XFFFFFFF0;// 清除PB0引脚的配置位   说明这是清除PB0引脚的配置位，并指出CRL寄存器低4位控制PB0
	// GPIOB->CRL |= 0X00000003; // LED(背光开关) = PB0  详细解释配置含义：推挽输出模式，最大速度50MHz，CNF=00, MODE=11
	// GPIOB->ODR |= 0x0001;	  // PB0输出高电平  说明PB0输出高电平点亮LCD背光，并指出ODR寄存器位0对应PB0
	...
	LCD_Display_Dir(1); // 屏幕方向
	LCD_Clear(WHITE);	// 清屏

	// // 缺少这个命令！
	// LCD_WR_REG(0x29); // 开启显示 - 应该添加
