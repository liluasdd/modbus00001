...
void LCD_Init(void)
{
	...
	gpio_initstruct.Pin = SPI_cs_pin; // B12
	HAL_GPIO_Init(SPI_cs_port, &gpio_initstruct);
	SPI_cs_HIGH_ON;

	gpio_initstruct.Pin = SPI_scK_pin; // A5
	HAL_GPIO_Init(SPI_scK_port, &gpio_initstruct);
	SPI_scK_HIGH_ON;

	gpio_initstruct.Pin = SPI_dc_pin; // A4
	HAL_GPIO_Init(SPI_dc_port, &gpio_initstruct);
	SPI_dc_HIGH_ON;

	gpio_initstruct.Pin = SPI_sdi_pin; // A7
	HAL_GPIO_Init(SPI_sdi_port, &gpio_initstruct);
	SPI_sdi_HIGH_ON;

	gpio_initstruct.Pin = SPI_led_pin; // B1
	...
	gpio_initstruct.Pin = SPI_sdo_pin;			   // A6
	HAL_GPIO_Init(SPI_sdo_port, &gpio_initstruct); //

	gpio_initstruct.Pin = T_IRQ_pin;			 // A6
	HAL_GPIO_Init(T_IRQ_port, &gpio_initstruct); // T_IRQ_pin

	SPI_reset_HIGH_ON; // 复位lcd
	delay_ms(1);
	SPI_reset_LOW_OFF;
	delay_ms(10);
	SPI_reset_HIGH_ON;
